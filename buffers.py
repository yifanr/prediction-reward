...
class RolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.episode_ends = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        super().reset()
    ...
    def add(
        ...
        self.episode_ends[self.pos] = np.array(episode_ends).copy()
        self.pos += 1
...
class RecurrentRolloutBuffer(RolloutBuffer):
    ...
    def __init__(
        ...
        feature_pred_shape: Tuple[int, int, int],
        device: Union[th.device, str] = "auto",
        gae_lambda: float = 1,
        gamma: float = 0.99,
        n_envs: int = 1,
        ...
        self.feature_pred_shape = feature_pred_shape
        self.seq_start_indices, self.seq_end_indices = None, None
        super().__init__(buffer_size, observation_space, action_space, device, gae_lambda, gamma, n_envs)

    def reset(self):
        super().reset()
        self.hidden_states = np.zeros(self.hidden_state_shape, dtype=np.float32)
        self.cell_states = np.zeros(self.hidden_state_shape, dtype=np.float32)
        self.next_observations = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=np.float32)
        self.episode_ends = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
    ...
    def add(
        ...
        **kwargs) -> None:
        ...
        """
        self.hidden_states[self.pos] = np.array(lstm_states[0].cpu().numpy())
        self.cell_states[self.pos] = np.array(lstm_states[1].cpu().numpy())
        self.next_observations[self.pos] = np.array(next_obs).copy()
        self.episode_ends[self.pos] = np.array(episode_ends).copy()
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[RecurrentRolloutBufferSamples, None, None]:
        ...
        if not self.generator_ready:
            ...
            for tensor in ["hidden_states", "cell_states"]:
    ...
    def _get_samples(
        ...
        lstm_states = (
            ...
            self.hidden_states[batch_inds][self.seq_start_indices].swapaxes(0, 1),
            self.cell_states[batch_inds][self.seq_start_indices].swapaxes(0, 1),
        )
        lstm_states = (self.to_torch(lstm_states[0]).contiguous(), self.to_torch(lstm_states[1]).contiguous())
        ...
        return RecurrentRolloutBufferSamples(
            ...
            episode_ends=self.pad_and_flatten(self.episode_ends[batch_inds]),
...
class RecurrentDictRolloutBuffer(DictRolloutBuffer):
    ...
    def add(self, *args, lstm_states: Tuple[th.Tensor, th.Tensor],
        next_obs: th.Tensor, 
        episode_ends: th.Tensor,
         **kwargs) -> None:
        """
    ...
    def _get_samples(
        ...
        lstm_states = (self.to_torch(lstm_states[0]).contiguous(), self.to_torch(lstm_states[1]).contiguous())
        features_dim = self.feature_pred_shape[-1]
