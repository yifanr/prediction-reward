...
class RecurrentPPO(OnPolicyAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        feature_prediction_shape = (self.n_envs, self.policy.features_dim)
        self._last_preds = th.zeros(feature_prediction_shape, device=self.device)
        self._last_error_preds = th.zeros(feature_prediction_shape, device=self.device)

        hidden_state_buffer_shape = (self.n_steps, lstm.num_layers, self.n_envs, lstm.hidden_size)
        feature_pred_buffer_shape = (self.n_steps, self.n_envs, self.policy.features_dim)
        ...
        while n_steps < n_rollout_steps:
            ...
            for idx, done_ in enumerate(dones):
                if (
                    ...
                    with th.no_grad():
                        terminal_lstm_state = (
                            lstm_states[0][:, idx : idx + 1, :].contiguous(),
                            lstm_states[1][:, idx : idx + 1, :].contiguous(),
                        )
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                # print(th.mean(rollout_data.returns))
                # print(th.mean(values_pred))
