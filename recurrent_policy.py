...
class RecurrentActorCriticPolicy(ActorCriticPolicy):
    ...
    def __init__(
        ...
        self.error_net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(lstm_hidden_size, self.features_dim),
            nn.ReLU(),
        )
    ...
    def forward(
    ...
    ) -> Tuple[th.Tensor, th.Tensor, th.Tensor, Tuple[th.Tensor, th.Tensor], th.Tensor, th.Tensor, th.Tensor]:
        """
        ...
        latent, lstm_states = self._process_sequence(features, lstm_states, episode_starts, self.lstm)
            # Re-use LSTM features but do not backpropagate
        # latent_vf = latent.detach()
    ...
    def predict_values(
        ...
        latent, _ = self._process_sequence(features, lstm_states, episode_starts, self.lstm)
        # latent_vf = latent.detach()
    ...
    def evaluate_actions(
        self, obs: th.Tensor, actions: th.Tensor, lstm_states: Tuple[th.Tensor, th.Tensor], episode_starts: th.Tensor, next_obs: th.Tensor, episode_ends: th.Tensor
        ...
        latent_pi = self.mlp_extractor.forward_actor(latent)
        latent_vf = self.mlp_extractor.forward_critic(latent.detach())
